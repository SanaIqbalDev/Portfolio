...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function addItem() {
    ...
    let id_ = getRandomNum(1, 10000, currentList) + "slider";
    ...
    image.src = "res/ic_new_item.png";

    const title = document.createElement("h3");
    title.className = "item-title";
    title.textContent = "NEW SERVICE " + (newItemIndex + 1);
    newItemIndex++;
  ...
  function FilterData() {
    ...
    const titleDataSet = GetUpdatedData();

    FillDataInDropDown(titleDataSet);
  ...
  function SearchSliderList(title) {
    ...
    for (const [index, item] of Object.entries(sliderItems)) {
      if (index < sliderItems.length - 1) {
        ...
        if (productTitle.textContent !== title) {
          const parentNode = itemTitle[index].parentNode.parentNode.parentElement;

          parentNode.removeChild(itemTitle.parentNode.parentElement);

...
app.get('/api/getServiceItem/:service_id', async (req, res) => {
  ...
  try {
    const getQuery = `SELECT title,description,image_url FROM SERVICES WHERE service_id = $1;`;
...
app.put('/api/updateServiceItem', async (req, res) => {
  ...
  const query = `UPDATE SERVICES SET title = $1, description = $2, image_url = $3 WHERE service_id = $4;`;
  const values = [
  ...
  if (result.rowCount === 1) {
    res.status(200).json(serviceItemData);
  } else {
    res.status(400).json({ error: 'Could not update service item' });
  }
});

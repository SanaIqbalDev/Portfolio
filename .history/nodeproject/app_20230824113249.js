...
app.get('/api/getAllPersons', (req, res) => {
  ...
  pool.query(selectQuery, (error, result) => {
    if (error) {
      ...
      res.status(500).json({ error: 'Internal server error' }).setHeader('Access-Control-Allow-Origin','*');
    } else {
      // Data was successfully fetched, and the result contains the selected data
      console.log('Data fetched successfully:', result.rows);
      res.status(200).json(result.rows).setHeader('Access-Control-Allow-Origin','*');
    }

...
import { getStorage , ref, uploadBytesResumable } from "https://www.gstatic.com/firebasejs/9.6.6/firebase-storage.js"; // Import Firebase Storage
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function uploadImage(file) {
    ...
    uploadTask.on('state_changed', (snapshot) => {
      ...
      // Upload complete, handle success here
  ...
  async function FetchIitialServicesData() {
    try {
      ...
      for (const item of result) {
        ...
        addItem(item.service_id, item.title, `data:image/png;base64,${item.image_url}`, item.description, false);
      }
  ...
  async function AddServiceData(newService) {
    ...
    const { arrayBuffer, fileName } = await fetchImageAsFile(newService.image_url);
    ...
    if (arrayBuffer) {
      ...
      formData.append('image_url', new Blob([arrayBuffer]), fileName)

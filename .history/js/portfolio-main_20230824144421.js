...
document.addEventListener("DOMContentLoaded", function () {
    ...
    for (const [index, item] of Object.entries(sliderItems)) {
      if (index < sliderItems.length - 1) {
        item.addEventListener("click", function () {
          ...
          FetchServiceDetailWithId(parseInt(item.parentElement.id))

          // displayServiceDetail(parseInt(item.parentElement.id));
        });
      }
    }
  }
  ...
  async function FetchServiceDetailWithId(service_id) {
    ...
    fetch(`http://localhost:3000/api/getServiceItem/${service_id}`, {
      ...
      .then(response => {
        ...
        // console.log('Item recieved successfully');
      }).then(data => {
        // console.log("response is:", data);
        // console.log('Fetched service data is :', data);
      })
      .catch(error => {
        // console.error('Error:', error);
      });

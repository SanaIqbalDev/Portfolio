...
app.post('/api/addService', async(req, res) => {
  ...
  const values = [serviceObject.title, serviceObject.description, serviceObject.image_url];
  ...
  pool.query(query,values, (error, result) => {
    if (error) {
      ...
      res.status(201).json({ message: 'Service added successfully', data: result.rows});
    }
  });
...
app.post('/api/addSkill', (req, res) => {
  ...
  const insertQuery = 'INSERT INTO SKILLS (skill_name) VALUES ($1) RETURNING *;';

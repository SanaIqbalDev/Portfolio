...
app.use(express.json()); // Use this line if you're using Express 4.16 or higher
...
//   const queryService = `INSERT INTO SERVICES (title, description, image_url) VALUES ($1, $2, $3) RETURNING *;`;
...
//   pool.query(queryService,values, (error,result) => {
//     if(error){
//       console.error('Error:', error);
//       res.status(500).json({ error: 'Internal Server Error' });
//     }
//     else{
//       console.log('Service added successfully:', result.rows);
...
app.post('/api/addService', async (req, res) => {
  const data = req.body;
  const query = `INSERT INTO SERVICES (title, description, image_url) VALUES ($1, $2, $3) RETURNING *;`;
  const values = [data.title, data.description, data.image_url];
